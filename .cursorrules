...
2. Create a change sub-folder for the work:
   - `<change-name>/<dimension-name>_<YYYY-MM-DD>.yaml` — new/modified dimension YAML, clean and copy-paste ready
   - `<change-name>/<dimension-name>_<YYYY-MM-DD>_comments.md` — what it does, why, paste location, dependencies, testing

The user pastes the dimension YAML into the production file. After applying, record what was done in `costformation-brain/my-org/context.md`.
